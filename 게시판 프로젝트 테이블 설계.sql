...
CREATE USER 'board'@'%' IDENTIFIED BY 'ksm123';
GRANT ALL PRIVILEGES ON `board`.* TO 'board'@'%';
FLUSH PRIVILEGES;

-- User 테이블 비밀번호를 암호화된 데이터를 넣기 위해 100으로 여유있게 설정
CREATE TABLE `User` (
...
CREATE TABLE `Article` (
	...
	`comment`	INT DEFAULT 0,`user`terms
...
-- SHA 암호화 함수 SHA2는 길이까지 지정
SELECT SHA2('1234',256); 


select * from `user` where `uid`= 'a103' and `pass`= SHA2('a3333',256);